...
@section('breadcrumb', 'Eventos / ' . $evento->title)
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex items-start justify-between">
            <div class="flex-1">
                <div class="flex items-center gap-3 mb-3">
                    <h1 class="text-4xl font-bold">{{ $evento->title }}</h1>
                    <span class="px-4 py-1.5 bg-white/20 backdrop-blur-sm text-white text-sm font-semibold rounded-full">
                        {{ $evento->getStatusLabel() }}
                    </span>
                </div>
                <p class="text-blue-100 mb-4 text-lg">{{ $evento->short_description }}</p>
                <div class="flex items-center gap-6 text-sm">
                    <div class="flex items-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                        </svg>
                        <span>{{ $evento->event_start_date->format('d M Y') }} - {{ $evento->event_end_date->format('d M Y') }}</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.eventos') }}" class="px-6 py-3 bg-white/10 hover:bg-white/20 backdrop-blur-sm rounded-xl transition-all font-medium">
                Volver
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
            ...
            <p class="text-3xl font-bold text-gray-900">{{ $evento->teams->count() }}</p>
            @if($evento->max_teams)
            <p class="text-sm text-gray-600 mt-1">de {{ $evento->max_teams }} máximo</p>
            @endif
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
            ...
            <p class="text-3xl font-bold text-gray-900">{{ $evento->projects->count() }}</p>
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
            ...
            <p class="text-3xl font-bold text-gray-900">{{ $evento->judges->count() }}</p>
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
            ...
            <p class="text-3xl font-bold text-gray-900">{{ $evento->teams->sum('members_count') }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100">
                ...
                <div class="p-6">
                    <p class="text-gray-700 leading-relaxed whitespace-pre-line">{{ $evento->description }}</p>
                </div>
            </div>
                    ...
                    <div class="p-6 hover:bg-gray-50 transition-colors">
                        <div class="flex items-start justify-between">
                            <div class="flex items-start gap-4 flex-1">
                                ...
                                <div class="flex-1">
                                    <h3 class="font-bold text-gray-900 mb-1">{{ $equipo->name }}</h3>
                                    <p class="text-sm text-gray-600 mb-2">{{ $equipo->description }}</p>
                                    <div class="flex items-center gap-4 text-sm">
                                        <span class="text-gray-600">
                                            <strong>Líder:</strong> {{ $equipo->leader->name }}
                                        </span>
                                        <span class="text-gray-600">
                                            <strong>Miembros:</strong> {{ $equipo->members_count }}
                                        </span>
                                        @if($equipo->project)
            ...
            <!-- Proyectos -->
            @if($evento->projects->count() > 0)
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100">
                <div class="p-6 border-b border-gray-100">
                    <div class="flex items-center justify-between">
                        <h2 class="text-2xl font-bold text-gray-900">Proyectos Presentados</h2>
                        <span class="px-3 py-1 bg-blue-100 text-blue-700 text-sm font-medium rounded-full">
                            {{ $evento->projects->count() }} proyectos
                        </span>
                    </div>
                </div>
                <div class="divide-y divide-gray-100">
                    @foreach($evento->projects as $proyecto)
                    <div class="p-6 hover:bg-gray-50 transition-colors">
                        <div class="flex items-start justify-between mb-3">
                            <div>
                                <h3 class="font-bold text-gray-900 mb-1">{{ $proyecto->title }}</h3>
                                <p class="text-sm text-gray-600 mb-2">{{ Str::limit($proyecto->description, 100) }}</p>
                                <p class="text-sm text-gray-600"><strong>Equipo:</strong> {{ $proyecto->team->name }}</p>
                            </div>
                            <span class="px-3 py-1 
                                @if($proyecto->status === 'evaluated') bg-green-100 text-green-700
                                @elseif($proyecto->status === 'submitted') bg-blue-100 text-blue-700
                        ...
                        <div class="flex items-center gap-2 mt-2">
                            ...
                            <span class="text-sm font-semibold text-gray-900">{{ number_format($proyecto->final_score, 1) }}/100</span>
                        </div>
                        @endif
                    </div>
                    @endforeach
        ...
        <div class="lg:col-span-1 space-y-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p class="text-sm font-semibold text-gray-900">
                            {{ $evento->registration_start_date->format('d M Y') }} - 
                            {{ $evento->registration_end_date->format('d M Y') }}
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-sm font-semibold text-gray-900">{{ ucfirst($evento->event_type ?? 'competition') }}</p>
                    </div>

                    @if($evento->contact_email)
                    <div>
                        <p class="text-sm text-gray-500 mb-1">Contacto</p>
                        <p class="text-sm font-semibold text-gray-900">{{ $evento->contact_email }}</p>
                    </div>
                    @endif

                    <div>
                        <p class="text-sm text-gray-500 mb-1">Modalidad</p>
                        <span class="inline-block px-3 py-1 {{ $evento->is_online ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700' }} text-xs font-medium rounded-full">
                            {{ $evento->is_online ? 'En línea' : 'Presencial' }}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
                ...
                <div class="space-y-3">
                    @forelse($evento->judges as $juez)
                    <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                        <div class="w-10 h-10 bg-purple-500 rounded-full flex items-center justify-center text-white font-bold">
                            {{ substr($juez->name, 0, 2) }}
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-semibold text-gray-900 truncate">{{ $juez->name }}</p>
                            <p class="text-xs text-gray-600 truncate">{{ $juez->email }}</p>
                        </div>
                    </div>
                    @empty
                ...
                <button onclick="openJudgesModal('{{ $evento->id }}', {{ json_encode($evento->judges->pluck('id')) }})" class="w-full mt-4 px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium rounded-xl transition-colors">
                    Gestionar Jueces
                </button>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
                ...
                <div class="space-y-3">
                    ...
                    <button onclick="confirmDelete('{{ $evento->id }}', '{{ $evento->title }}')" class="w-full px-4 py-3 text-sm font-medium text-red-700 hover:bg-red-50 rounded-xl transition-colors flex items-center justify-between">
