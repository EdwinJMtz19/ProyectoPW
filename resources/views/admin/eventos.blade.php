            ...
            <div class="p-6">
                <div class="flex items-start justify-between mb-4">
                    <div class="flex items-start gap-4 flex-1">
                        <div class="w-20 h-20 bg-gradient-to-br from-{{ $evento->status === 'in_progress' ? 'green' : ($evento->status === 'open' ? 'blue' : 'gray') }}-500 to-{{ $evento->status === 'in_progress' ? 'green' : ($evento->status === 'open' ? 'blue' : 'gray') }}-600 rounded-xl flex items-center justify-center text-3xl shadow-md flex-shrink-0">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="mt-4 flex items-center gap-4">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    @if($evento->advisors_count > 0)
                                    <span class="text-sm text-gray-600">{{ $evento->advisors_count }} asignados</span>
                                    <button onclick="openAdvisorsModal('{{ $evento->id }}', {{ json_encode($evento->advisors->pluck('id')) }})" class="text-sm text-blue-600 hover:text-blue-700 font-medium">
                                        Gestionar
                                    </button>
                                    @else
                                    <button onclick="openAdvisorsModal('{{ $evento->id }}', [])" class="text-sm text-orange-600 hover:text-orange-700 font-medium">
                                        Asignar Asesores
                                    </button>
                                    @endif
                                </div>
...
<div id="createModal" class="hidden fixed inset-0 bg-gray-900 bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form action="{{ route('admin.eventos.crear') }}" method="POST" class="p-6">
            @csrf
            <div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl max-w-2xl w-full">
        ...
        <form id="advisorsForm" method="POST" class="p-6">
            ...
            <div class="space-y-4 max-h-96 overflow-y-auto">
                ...
                <label class="flex items-center p-4 bg-gray-50 rounded-xl hover:bg-gray-100 cursor-pointer transition-colors">
                    <input type="checkbox" name="advisors[]" value="{{ $asesor->id }}" class="advisor-checkbox w-4 h-4 text-gray-900 border-gray-300 rounded focus:ring-gray-900">
                    <div class="ml-4">
                        <p class="text-sm font-medium text-gray-900">{{ $asesor->name }}</p>
                        <p class="text-xs text-gray-600">{{ $asesor->email }}</p>
                    </div>
                </label>
                @endforeach
            </div>
