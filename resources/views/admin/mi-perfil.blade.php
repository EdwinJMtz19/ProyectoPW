    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
                <div class="text-center mb-6">
                    ...
                    <h2 class="text-2xl font-bold text-gray-900 mb-1">{{ $usuario->name }}</h2>
                    <p class="text-gray-600 mb-3">{{ $usuario->email }}</p>
                ...
                <div class="space-y-3 pt-6 border-t border-gray-100">
                    ...
                    <div class="flex items-center gap-3 text-sm">
                        ...
                        <span class="font-semibold text-gray-900">{{ $usuario->numero_control }}</span>
                    </div>
                    @endif
                    <div class="flex items-center gap-3 text-sm">
                        <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <span class="font-semibold text-gray-900">{{ $usuario->created_at->format('M Y') }}</span>
                    </div>
                    <div class="flex items-center gap-3 text-sm">
                        <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <span class="font-semibold text-gray-900">{{ $usuario->updated_at->diffForHumans() }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between p-3 bg-blue-50 rounded-xl">
                        ...
                        <span class="text-2xl font-bold text-blue-600">{{ $totalEventos }}</span>
                    </div>
                    ...
                    <div class="flex items-center justify-between p-3 bg-purple-50 rounded-xl">
                        ...
                        <span class="text-2xl font-bold text-purple-600">{{ $totalUsuarios }}</span>
                    </div>
                    ...
                    <div class="flex items-center justify-between p-3 bg-green-50 rounded-xl">
                        ...
                        <span class="text-2xl font-bold text-green-600">{{ $totalEquipos }}</span>
                    </div>
                    ...
                    <div class="flex items-center justify-between p-3 bg-orange-50 rounded-xl">
                        ...
                        <span class="text-2xl font-bold text-orange-600">{{ $totalProyectos }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100">
                ...
                <form action="{{ route('admin.perfil.actualizar') }}" method="POST" class="p-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $usuario->name) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Correo Electrónico *</label>
                            <input type="email" name="email" value="{{ old('email', $usuario->email) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Biografía</label>
                            <textarea name="bio" rows="4" placeholder="Escribe algo sobre ti..." class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all">{{ old('bio', $usuario->bio) }}</textarea>
                        </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100">
                ...
                <form action="{{ route('admin.perfil.actualizar-password') }}" method="POST" class="p-6">
                    @csrf
                    @method('PUT')
