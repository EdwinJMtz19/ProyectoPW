...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <!-- Filtros -->
    <form method="GET" action="{{ route('admin.rankings') }}" class="bg-white rounded-2xl shadow-sm p-6 mb-8 border border-gray-100">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100">
                ...
                <div class="divide-y divide-gray-100">
                    ...
                    <div class="p-6 hover:bg-gray-50 transition-colors cursor-pointer" onclick="viewProjectDetails('{{ $proyecto->id }}')">
                        <div class="flex items-center gap-4">
                            <!-- Posición -->
                            <div class="flex-shrink-0">
                                @if($proyecto->display_rank <= 3)
                                <div class="w-16 h-16 rounded-full flex items-center justify-center
                                    {{ $proyecto->display_rank == 1 ? 'bg-gradient-to-br from-yellow-400 to-yellow-500' : 
                                       ($proyecto->display_rank == 2 ? 'bg-gradient-to-br from-gray-300 to-gray-400' : 
                                        'bg-gradient-to-br from-orange-400 to-orange-500') }}
                                    shadow-lg">
                                    <span class="text-2xl font-bold text-white">{{ $proyecto->display_rank }}</span>
                                </div>
                                @else
                                <div class="w-16 h-16 rounded-full flex items-center justify-center bg-gray-100">
                                    <span class="text-2xl font-bold text-gray-700">{{ $proyecto->display_rank }}</span>
                                </div>
                                @endif
                            </div>
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between gap-4">
                                    <div class="flex-1">
                                        <h3 class="text-lg font-bold text-gray-900 mb-1">{{ $proyecto->title }}</h3>
                                        <p class="text-sm text-gray-600 mb-2">{{ $proyecto->team->name }}</p>
                                        <div class="flex items-center gap-4">
                                            <div class="flex items-center gap-1 text-sm text-gray-600">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path>
                                                </svg>
                                                <span>{{ $proyecto->team->members_count }} integrantes</span>
                                            </div>
                                            <div class="flex items-center gap-1 text-sm text-gray-600">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                                </svg>
                                                <span>{{ $proyecto->event->title }}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex-shrink-0 text-right">
                                        <div class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl text-white">
                                            ...
                                            <span class="text-2xl font-bold">{{ number_format($proyecto->final_score, 1) }}</span>
                                        </div>
                                ...
                                <div class="mt-3 flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium text-gray-700">{{ $proyecto->team->leader->name }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
                ...
                <div class="space-y-4">
                    <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-4">
                        ...
                        <p class="text-3xl font-bold text-blue-900">{{ number_format($proyectos->avg('final_score'), 1) }}</p>
                    </div>

                    <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-4">
                        <p class="text-sm text-green-700 mb-1">Puntuación Más Alta</p>
                        <p class="text-3xl font-bold text-green-900">{{ number_format($proyectos->max('final_score'), 1) }}</p>
                    </div>

                    <div class="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-4">
                        <p class="text-sm text-purple-700 mb-1">Proyectos Evaluados</p>
                        <p class="text-3xl font-bold text-purple-900">{{ $proyectos->count() }}</p>
                    </div>
                    ...
                    <div class="bg-gradient-to-br from-orange-50 to-orange-100 rounded-xl p-4">
                        ...
                        <p class="text-3xl font-bold text-orange-900">{{ $proyectos->unique('team_id')->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-2xl shadow-sm p-6 border border-yellow-200">
                ...
                <div class="space-y-3">
                    @foreach($proyectos->take(3) as $top)
                    <div class="bg-white/80 backdrop-blur rounded-xl p-3">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-full flex items-center justify-center
                                {{ $top->display_rank == 1 ? 'bg-yellow-400' : ($top->display_rank == 2 ? 'bg-gray-300' : 'bg-orange-400') }}">
                                <span class="text-sm font-bold text-white">{{ $top->display_rank }}</span>
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-bold text-gray-900 truncate">{{ Str::limit($top->team->name, 20) }}</p>
                                <p class="text-xs text-gray-600">{{ number_format($top->final_score, 1) }} pts</p>
                            </div>
                        </div>
                    </div>
                    @endforeach
                ...
                <div class="space-y-3">
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <span class="text-sm font-medium text-gray-900">{{ $proyectos->where('final_score', '>=', 90)->count() }}</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-green-500 h-2 rounded-full" style="width: {{ ($proyectos->where('final_score', '>=', 90)->count() / max($proyectos->count(), 1)) * 100 }}%"></div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <span class="text-sm font-medium text-gray-900">{{ $proyectos->whereBetween('final_score', [80, 89])->count() }}</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-blue-500 h-2 rounded-full" style="width: {{ ($proyectos->whereBetween('final_score', [80, 89])->count() / max($proyectos->count(), 1)) * 100 }}%"></div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <span class="text-sm font-medium text-gray-900">{{ $proyectos->whereBetween('final_score', [70, 79])->count() }}</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-yellow-500 h-2 rounded-full" style="width: {{ ($proyectos->whereBetween('final_score', [70, 79])->count() / max($proyectos->count(), 1)) * 100 }}%"></div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <span class="text-sm font-medium text-gray-900">{{ $proyectos->where('final_score', '<', 70)->count() }}</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-red-500 h-2 rounded-full" style="width: {{ ($proyectos->where('final_score', '<', 70)->count() / max($proyectos->count(), 1)) * 100 }}%"></div>
                        </div>
                    </div>
                </div>
            </div>
