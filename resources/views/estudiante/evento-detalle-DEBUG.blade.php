...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <!-- DEBUG: Mostrar qué equipo tiene el usuario -->
    @if(config('app.debug'))
        <div class="bg-yellow-100 border border-yellow-400 text-yellow-800 px-4 py-3 rounded mb-4">
            <strong>DEBUG:</strong>
            @if($miEquipo)
                Mi equipo ID: {{ $miEquipo->id }} | Nombre: {{ $miEquipo->name }}
            @else
                No tengo equipo en este evento
            @endif
        </div>
    @endif
                                    ...
                                    <div class="flex-1">
                                        ...
                                        @if(config('app.debug'))
                                            <p class="text-xs text-gray-400 mt-2">
                                                DEBUG: Equipo ID: {{ $equipo->id }} 
                                                @if($miEquipo) | Mi equipo ID: {{ $miEquipo->id }} @endif
                                                | Match: {{ $miEquipo && $miEquipo->id === $equipo->id ? 'SÍ' : 'NO' }}
                                            </p>
                                        @endif
                                    </div>
...
<!-- Modales y JavaScript igual que antes -->
