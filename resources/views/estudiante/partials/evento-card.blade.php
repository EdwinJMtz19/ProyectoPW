<div class="evento-card bg-white rounded-lg shadow-sm hover:shadow-md transition-all overflow-hidden border border-gray-200" 
    ...
    <div class="relative h-48">
        @if($evento->getCoverImageUrl())
            <img src="{{ $evento->getCoverImageUrl() }}" alt="{{ $evento->title }}" class="w-full h-full object-cover">
        @else
        ...
        <div class="space-y-2.5 mb-5 text-sm text-gray-600">
            <div class="flex items-center gap-2.5">
                ...
                <span class="text-gray-700">
                    @if($tipo === 'proximo')
                        Inicia: {{ \Carbon\Carbon::parse($evento->event_start_date)->format('d M Y') }}
                    @elseif($tipo === 'activo')
                        Finaliza: {{ \Carbon\Carbon::parse($evento->event_end_date)->format('d M Y') }}
                    @else
                        Finalizó: {{ \Carbon\Carbon::parse($evento->event_end_date)->format('d M Y') }}
                    @endif
                </span>
            </div>
            <div class="flex items-center gap-2.5">
