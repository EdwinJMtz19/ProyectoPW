...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6 mb-8 border border-gray-100">
        <form method="GET" action="{{ route('juez.eventos') }}">
            <div class="flex flex-col md:flex-row gap-4">
                ...
                <div class="w-full md:w-48">
                    ...
                    <select name="status" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                        ...
                        <option value="upcoming" {{ request('status') == 'upcoming' ? 'selected' : '' }}>Próximamente</option>
                ...
                <div class="bg-white rounded-2xl shadow-sm overflow-hidden hover:shadow-lg transition-all duration-300 border border-gray-100">
                    <div class="relative h-48">
                        ...
                        <div class="absolute top-4 left-4">
                            <span class="px-3 py-1.5 {{ $statusBadge }} text-white text-xs font-semibold rounded-full shadow-lg">
                                {{ $statusText }}
                            </span>
                        </div>
                    ...
                    <div class="p-6">
                        <div class="flex items-center gap-4 mb-4 text-sm text-gray-600">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <span class="font-medium">{{ $event->projects_count }} proyecto{{ $event->projects_count != 1 ? 's' : '' }}</span>
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            <div class="flex justify-between text-sm mb-2">
                                ...
                                <span class="font-bold {{ $event->evaluation_progress == 100 ? 'text-green-600' : 'text-indigo-600' }}">
                                    {{ $event->evaluated_projects_count }}/{{ $event->projects_count }}
                                </span>
                            </div>
                            <div class="w-full bg-gray-200 rounded-full h-2">
                                <div class="h-2 rounded-full transition-all duration-500 {{ $event->evaluation_progress == 100 ? 'bg-green-600' : 'bg-indigo-600' }}" 
                                     style="width: {{ $event->evaluation_progress }}%"></div>
                            </div>
                        </div>
                        
                        @if($isCompleted)
                            <a href="{{ route('juez.rankings', ['event_id' => $event->id]) }}" 
                        ...
                        @else
                            <a href="{{ route('juez.evaluaciones', ['event_id' => $event->id]) }}" 
